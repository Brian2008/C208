...
def openChatWindow():
    ...
    text_message = Entry(window,width=43,font = ("Calibiri",12))
    text_message.pack()
    text_message.place(x=98,y=306)

    send = Button(window,text="Send",bd=1, font=("Calibiri",10))
    send.place(x=450,y=305)

    filePathLabel = Label(window,text="", fg="blue", font=("Calibiri",8))
